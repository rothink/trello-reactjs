...
gulp.task('b-img', () => {
    return 
        gulp
            ...
            .pipe(changed(PATH.buildDir.concat('/imgages')))
            .pipe(gulp.dest(PATH.buildDir.concat('/imgages')))
            .pipe(livereload())
})

gulp.task('watch', () => {
    gulp.watch('src/*.html', ['b-html'])
    gulp.watch('src/styles/*.css', ['b-css'])
